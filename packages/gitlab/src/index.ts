...
const MergeRequestWebhook = Schema.Struct({
  object_attributes: Schema.optional(Schema.Struct({
...
const GitLabChanges = Schema.Struct({
  changes: Schema.optional(Schema.Array(Schema.Struct({
...
export const createGitLabTasks = (options: GitLabOptions) => {
  const request = async <S extends Schema.ConstraintDecoder<unknown, never>>(path: string, schema: S, init?: RequestInit): Promise<S["Type"]> => {
  ...
  const listOpenMergeRequests: TaskNode<void, MergeRequestEvent[]> = Task.fn({
    ...
    run: () => Effect.tryPromise(async () => {
      ...
      const requests = await request(`${`/projects/${project}/merge_requests?state=opened&scope=all&per_page=100`}`, Schema.Array(GitLabMergeRequest))
      return requests.map((mr) => ({ projectId: options.projectId!, mergeRequestIid: mr.iid, action: "manual", title: mr.title }))
    }),
  })
  ...
  const getMergeRequestContext: TaskNode<MergeRequestEvent, MergeRequestContext> = Task.fn({
    ...
    run: (event) => Effect.tryPromise(async () => {
      ...
      const changes = await request(`/projects/${project}/merge_requests/${event.mergeRequestIid}/changes`, GitLabChanges)
      return {
        ...event,
        title: mr.title,
        description: mr.description ?? "",
        diff: (changes.changes ?? []).map((change) => `--- ${change.old_path ?? ""}\n+++ ${change.new_path ?? ""}\n${change.diff ?? ""}`).join("\n"),
  ...
  const createReviewComment: TaskNode<{ event: MergeRequestEvent; review: GitLabReview }, void> = Task.effect({
    ...
    run: ({ event, review }) => Effect.tryPromise(async () => {
      const project = encodeURIComponent(event.projectId)
...
function formatReview(review: GitLabReview): string {
  const findings = review.findings.length === 0 ? "No findings." : review.findings.map((finding) => `- **${finding.severity}**${finding.file ? ` \`${finding.file}\`` : ""}: ${finding.message}`).join("\n")
