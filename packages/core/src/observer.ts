...
export interface SchedulerObserver {
  ...
  nodeCompleted(event: { readonly runId?: RunId; readonly nodeId: NodeId; readonly output: unknown; readonly durationMs: number }): void
  nodeFailed(event: { readonly runId?: RunId; readonly nodeId: NodeId; readonly error: unknown; readonly durationMs: number }): void
  workflowCompleted(event: { readonly runId: RunId; readonly workflowId: NodeId; readonly status: "completed" | "failed" }): void
}

export const SchedulerObservers = {
  noop: {
    triggerReceived: () => {}, workflowScheduled: () => {}, nodeStarted: () => {},
    nodeCompleted: () => {}, nodeFailed: () => {}, workflowCompleted: () => {},
...
export function notify(callback: () => void): void {
  try { callback() } catch { /* intentionally ignored */ }
}
