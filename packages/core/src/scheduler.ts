...
export const createScheduler = <Input = void, Output = unknown>(options: { workflows: readonly Node<Input, Output>[]; observer: SchedulerObserver }): Scheduler<Input, Output> => {
  ...
  return {
    ...
    async runTrigger(triggerId, value) {
      ...
      return Promise.all(workflows.map(async (workflow) => {
        ...
        try {
          const output = await Effect.runPromise(workflow.execute(undefined as Input, {
            triggerValues: new Map([[triggerId, value]]), runId: id, nodeId: workflow.id, observer: options.observer,
          ...
          const run = { id, workflowId: workflow.id, triggerId, status: "failed", error: serializeExecutionError(error) } as WorkflowRun
          notify(() => options.observer.workflowCompleted({ runId: id, workflowId: workflow.id, status: run.status }))
          return run
        }
      }))
    },
    run() { throw new Error("No HTTP/runtime adapter configured. Use @libclank/hono or call runTrigger().") },
  }
}
