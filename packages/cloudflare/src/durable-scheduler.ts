...
export class CloudflareWorkflowRun {
  ...
  async tick(runId: RunId): Promise<void> {
    const rows = this.state.storage.sql
      .exec<{ node_id: string; input: string; attempt: number }>(
        "SELECT node_id,input,attempt FROM durable_nodes WHERE status='ready' AND (next_attempt_at IS NULL OR next_attempt_at <= ?)",
        Date.now(),
      )
      .toArray()
    for (const row of rows) {
      ...
      this.state.storage.sql.exec(
        "UPDATE durable_nodes SET status='running', attempt=? WHERE node_id=? AND status='ready'",
        attempt,
        row.node_id,
      )
      try {
        ...
        this.state.storage.sql.exec(
          "UPDATE durable_nodes SET status='completed',output=? WHERE node_id=?",
        ...
        this.state.storage.sql.exec(
          "UPDATE durable_nodes SET status=?,next_attempt_at=?,error=? WHERE node_id=?",
          attempt < 3 ? "ready" : "failed",
          retryAt,
  ...
  async alarm(runId: RunId): Promise<void> {
    await this.tick(runId)
  }

  private append(type: string, payload: unknown): void {
    this.state.storage.sql.exec(
      "INSERT INTO durable_events (type,payload,created_at) VALUES (?,?,?)",
      type,
